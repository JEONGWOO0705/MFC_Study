...
// mfc_BitmapBtnDlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "mfc_BitmapBtn.h"
#include "mfc_BitmapBtnDlg.h"
...
CmfcBitmapBtnDlg::CmfcBitmapBtnDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFC_BITMAPBTN_DIALOG, pParent)
{
...
BOOL CmfcBitmapBtnDlg::OnEraseBkgnd(CDC* pDC)
{
	...
	CBitmap* p0idBitmap = dc.SelectObject(&image);

	pDC->BitBlt(0, 0, 640, 480, &dc, 0, 0, SRCCOPY);
	dc.SelectObject(p0idBitmap);
	return TRUE;
...
void  CmfcBitmapBtnDlg::InitButtons() {
	...
	GetDlgItem(IDC_BTN_ON_OFF)->GetWindowRect(&rect); //현재 rect의 위치를 들고와줌
	//GetDlgItem(IDC_BTN_ON_OFF)->GetClientRect(&rect); //현재 rect의 위치를 들고와줌
