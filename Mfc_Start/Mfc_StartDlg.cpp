...
#pragma comment(linker, "/entry:WinMainCRTStarup /subsystem:console")
#endif
...
CMfcStartDlg::CMfcStartDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFC_START_DIALOG, pParent)
	, m_nNum(0) // 초기 숫자값 입력
	, m_nNum2(0)
{
...
void CMfcStartDlg::OnBnClickedBtnTest()
{
	...
	CString result;
	result.Format(_T("%d"), m_nNum + m_nNum2);
	SetDlgItemText(IDC_STATIC_RESULT, result);

	UpdateData(false);  // 버튼을 눌렀을떄 숫자가 변화됨
}
